...
import { downloadContentFromMessage, encryptedStream, generateThumbnail, getAudioDuration, getAudioWaveform, getRawMediaUploadData, buildZipArchive, resolveStickerBuffer } from './messages-media.js';
...
export const generateWAMessageContent = async (message, options) => {
    ...
    if ('interactiveButtons' in message && !!message.interactiveButtons) {
        ...
        const trayIconPng = await convertToTrayIcon(await resolveStickerBuffer((rawStickers[0] && typeof rawStickers[0] === 'object' && !Buffer.isBuffer(rawStickers[0]) && 'image' in rawStickers[0])
            ? rawStickers[0].image
            : rawStickers[0], options.options));
