...
export const extractDeviceJids = (result, myJid, myLid, excludeZeroDevices) => {
    ...
    // `myLid` is a full JID (e.g. "12345@lid"); decode it to a bare user id
    // before comparing against `user` below (also a bare user id) — comparing
    // the raw JID directly was always false for a same-user LID device,
    // which could misclassify your own LID-addressed device as "a different
    // user" during device-list fan-out for encryption.
    const myLidUser = myLid ? jidDecode(myLid)?.user : null;
    const extracted = [];
