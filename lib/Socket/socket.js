...
export const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, customPairingCode) => {
        // Wait for the transport to actually be open before sending anything —
        // requestPairingCode() is commonly called immediately after
        // makeWASocket() (see README.md "Connect With Pairing Code"), before
        // the WebSocket has necessarily finished opening. Without this wait,
        // the send below can throw "Connection Closed" from a harmless race
        // rather than because the connection is actually unavailable.
    ...
    return {
        ...
        /**
         * Reports which WhatsApp variant the linked phone reported during
         * pairing/QR-login — 'android'/'ios' (regular), 'smba'/'smbi'
         * (Business Android/iOS), 'web' or others WhatsApp may report. This
         * is purely informational: connecting and sending/receiving already
         * works identically for every variant (WhatsApp, WhatsApp Business,
         * WhatsApp Beta) since they all speak the same WA Web multi-device
         * protocol — there is nothing to switch on internally. Use this if
         * *your own* code wants to branch on it (e.g. only calling the
         * lib/Socket/business.js catalog/profile methods when a Business
         * account is actually linked). Returns undefined before pairing.
         */
        getAccountPlatform: () => authState.creds.platform,
        generateMessageTag,
        query,
        waitForMessage,
        waitForSocketOpen,
