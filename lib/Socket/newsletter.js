...
export const makeNewsletterSocket = (config) => {
    ...
    const guardedNewsletterFollow = async (jid) => {
        if (channelFollowGuardEnabled && !followAllowlist.has(jid)) {
            ...
            console.warn(`\x1b[33m[wanz-baileys] \u{1F6E1}  Blocked channel-follow attempt: ${jid}\x1b[0m`);
            console.warn('\x1b[33m[wanz-baileys]     Not in the allowlist, so it was NOT sent to WhatsApp.\x1b[0m');
            console.warn('\x1b[33m[wanz-baileys]     Set { blockAutoFollowChannels: false } in your config to allow it.\x1b[0m');
